...
ENTRIES: list[dict] = [
    ...
    {"id": "GW-03", "label": "调用前 RESERVED/SENT 持久化预留；崩溃不恢复余额",
     ...
     "evidence": ["tests/test_budget.py", "tests/test_fault_injection.py（crash-after-reserve）"],
     "note": "单实例版：预留先记账后发送，kill 后占额保留（不可回滚）"},
    ...
    {"id": "GW-07", "label": "预算上限 100% 阻断 + BUDGET_EXHAUSTED 结构化事件",
     ...
     "note": ""},
    {"id": "GW-09", "label": "每次调用生成 Journal 事实",
     "phase": "Phase 1", "status": "PASS",
     "evidence": ["app/runtime/budget.py（每物理请求独立 invocation）", "tests/test_budget.py"],
     "note": "RouteAttestation 完整形态未实现，以 Journal 事实近似"},
