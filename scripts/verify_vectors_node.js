// 独立参考实现比对：对 contracts/test-vectors 的全部正向量，
// 用 Node 参考实现重算 canonicalPayloadB64 / payloadDigest，与固化向量逐字节比对。
// 任一不一致即退出码 1（对应手册 CT-01：Go 主实现与独立参考实现逐字节一致）。
'use strict';
const fs = require('fs');
...
const vectorFile = path.join(ROOT, 'contracts', 'test-vectors', 'attempt_contract', 'v2', 'vectors.json');
const profileFile = path.join(ROOT, 'contracts', 'jsonschema', 'attempt_contract.v2.digestprofile.json');
...
for (const v of vectors.vectors) {
  ...
  if (!payloadOk || !digestOk) {
    failed++;
    console.error(`[MISMATCH] ${v.id}: payloadOk=${payloadOk} digestOk=${digestOk}`);
  } else {
    console.log(`[OK] ${v.id} payload=${payload.length}B digest=${digest.slice(0, 19)}...`);
  }
}
