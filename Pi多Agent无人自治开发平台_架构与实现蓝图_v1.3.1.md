...
文档状态：设计基线候选稿（v1.3 第三方评审整改稿，待复审签署）  
基线日期：2026-09-05  
适用范围：内部单租户；首个兑现阶段仅处理 PUBLIC / 已脱敏代码任务  
前序基线：v1.2 与 v1.3 保留不改；v1.3.1 是 v1.3 的评审整改增量，三者及本文件的“继承清单”共同构成 v1.3.1 规范  
本轮交付范围：Markdown 主文档。可视化图不属于 v1.3.1 规范性基线；若后续单独制作，只能作为本文的派生视图，不能新增或改变权威关系。
...
按至少 5 名核心 FTE 并配独立安全 / 评测投入估算，经 v1.3 第三方评审后修正：Phase 0 至 Phase 2 的首个内部 MVP 预计需要 **20～28 周**（原 13～19 周被评审判定为偏乐观；该区间为含跨阶段集成、评审与缓冲的规划工期，各阶段名义时长合计 16～23 周，计算口径见 §19.2/§21）；到 Phase 4.5 的有限生产可用候选，不含可选 HA 预计 **40～58 周**，含 Phase 4B HA 预计 **45～66 周**。工作可以并行，但阶段门槛不能跳过。
...
- 数据分级与外部副作用声明；
- 候选分支命名和人工评审责任人；
- **离线可行性声明（v1.3.1 明确）：Phase 2 白名单任务的全部 Gate 必须完全离线运行、不需要新增未经预构建的依赖**；运行时发现依赖缺失只能产生 `DependencyChangeProposal`（§6.14），不能进入当前 Attempt。
...
## 2. v1.2 到 v1.3 的收敛变化与 v1.3.1 评审整改

v1.2 已建立 Task / Run / Attempt、Lease / Fencing、Route Attestation、Verification Plane、Artifact、Git Saga、预算与安全治理的主体框架。v1.3 不推翻这些约束，而是将其组织成可实现的仓库、组件、接口、供应链和阶段交付计划。v1.3.1 在 v1.3 基础上关闭第三方评审发现的 7 项 P0 内部矛盾、11 项 P1 设计缺口，并修正工期与样本量口径。
...
| 人员约束 | Four-Eyes 为关键变更要求 | 单人条件下只能发布 PoC 结论，不能进入生产 ACTIVE 或 Canary |
| v1.3.1 评审整改 | — | 关闭 7 项 P0 内部矛盾（撤销水位统一、附录 B 与正文对齐、Repair 回路、事件单一发布者、政策模板与任务实例分离、预算热路径、政策审批对象）；补齐 11 项 P1 缺口；修正工期为 20～28 周并引入 MUST-P2 / MUST-P4A 分层 |

### 2.1 v1.3 评审整改摘要

v1.3 作为设计基线候选稿接受了第三方独立评审。评审总体结论为“方向正确、不能直接批准”，并在签署前要求关闭以下事项（完整闭环见第 28 章评审整改闭环表与本文件交付说明）：

- **P0 七项（阻塞签署，本节已全部修订为正文条款）**：撤销水位模型正文与附录不一致；附录 B 结构与枚举与 §10.6 冲突；Task 状态机缺 Repair 回路；TaskAccepted / TaskRejected 双发布者；政策 Four-Eyes 与逐任务实例混为一谈；预算检查与热路径约束冲突；GatePack / Prompt / ToolPolicy / RolePack / Workflow 缺审批对象。
- **P1 十一项（缺口修复或进入 §27.1 门槛清单）**：Prime 零网络可行性前置 spike；候选分支评审材料（PR/MR）副作用语义；candidate ref 多轮 Repair 更新策略；Reviewer 独立性的可编译判定；Semantic Review 信任域归属；Commit Assembler 时序；Attempt 优雅终止路径；任务白名单离线能力要求；Lease TTL 冻结；Skill 来源枚举对齐；Node 对 AttemptTerminalEnvelope 的签名密钥。
- **计划与可行性**：Phase 0～2 工期修正为 20～28 周；新增 MUST-P2 / MUST-P4A 规范分层；负例样本量与统计上界写入各阶段门槛；同宿主旁路风险按阶段强化隔离。
- **文档质量**：新增术语表（附录 E）、v1.2 继承清单（附录 F）、Schema 派生对象依赖 DAG（附录 G），并修正 §5.1 标题与图表表述。

评审明确肯定的设计在 v1.3.1 中原样保留，不得在后续“简化”中删除：确定性 Gate 由宿主 waitpid / cgroup / 流式哈希捕获、Git 写入完全移出沙箱、三个 Bundle 摘要语义分离、APPLYING 不等于成功且取消先对账、单人组织只能标 PoC、SkillRoster 严格离线只读。

### 2.2 评审处置矩阵

| 评审项 | 级别 | 处置结论 | 修订章节 | 规范等级 | 是否阻塞签署 |
|---|---|---|---|---|---|
| 撤销水位模型：全局 epoch vs 附录 B 分域 epoch | P0 | 统一为单一全局 revocationEpoch，删除全部分域水位 | §10.6 / §10.7 / 附录 B | MUST-P2 | 是 |
| 附录 B 与 §10.6 结构、枚举不一致 | P0 | 附录 B 改为由 §10.6 扁平 Schema 生成的规范正例，并接受 CI 校验 | 附录 B / §17.4 | MUST-P2 | 是 |
| Task 状态机缺 Repair 回路 | P0 | 新增 VERIFYING→REPAIR_PLANNING→EXECUTING 回路（Repair Run 血缘见 §14.3） | §8.1 / §8.2 / §14.3 / §10.5.1 | MUST-P2 | 是 |
| TaskAccepted / TaskRejected 双发布者 | P0 | Task API 只发 TaskAdmissionDecided；Lifecycle 发布 TaskAccepted / TaskRejected | §6.1 / §8.1 / §13.6 | MUST-P2 | 是 |
| 政策 Four-Eyes 与任务实例混用 | P0 | 拆分政策模板（Four-Eyes）与确定性编译的任务实例（免人工） | §7.7 / §10.3.1 / §14.2 | MUST-P2 | 是 |
| 预算检查与热路径冲突 | P0 | 引入签名 BudgetGrant，Gateway 本地单调扣减、异步结算、Fail Closed | §12.5 / §13.3 / §18.3 / §26.1 | MUST-P2 | 是 |
| GatePack / Prompt / ToolPolicy / RolePack / Workflow 缺审批对象 | P0 | 扩展 PolicyApprovalScopeSnapshot.subjectType 闭集并定义 RolePack.approvalRefs 合同 | §10.3.1 / §10.5 / §26.1 | MUST-P2 | 是 |
| Prime 零网络可行性 | P1 | Phase 0 增加 spike，纳入风险登记；不通过则切换 Native Pi / Scripted | §19.3 / §23 | MUST-P2 | 是（列入门槛） |
| 评审材料 PR/MR 副作用 | P1 | Phase 2 默认只创建候选 ref 与评审材料；PR 创建作为独立外部副作用单列 | §3.5 / §12.8 / §19.5 / §23 | MUST-P2 | 否 |
| candidate ref 多轮 Repair 更新策略 | P1 | 首次创建 CAS 语义与后续更新基线写入 §10.10 | §8.5 / §10.10 | MUST-P2 | 否 |
| Reviewer 独立性判定规则 | P1 | 给出可编译期校验的独立性规则 | §6.3 / §19.6 | MUST-P2 | 否 |
| Semantic Review 信任域归属 | P1 | 编排属 Domain 1，LLM 执行走独立 Review Run（Domain 3/4/5），Domain 8 不调 LLM | §4 / §6.10 | MUST-P2 | 否 |
| Commit Assembler 时序 | P1 | 新增 Run 状态 COMMIT_ASSEMBLING | §8.2 / §6.10 | MUST-P2 | 否 |
| Attempt 优雅终止 | P1 | 新增 TERMINATING → TERMINAL_REPORTED | §8.3 / §12.2 | MUST-P2 | 否 |
| 任务白名单离线能力 | P1 | Phase 2 全部 Gate 离线；依赖缺失只能产生 DependencyChangeProposal | §1.2 / §6.14 / §19.5 | MUST-P2 | 否 |
| Lease TTL 未冻结 | P1 | Phase 0 冻结 LeasePolicySnapshot（初值 Execution Lease TTL 30s、heartbeat 10s） | §6.5 / §18.2 / §19.3 | MUST-P2 | 否 |
| SkillPackageVersion.sourceKind 枚举 | P1 | 增加 PUBLISHED_ARTIFACT 及其 digest / provenance 要求 | §10.1 / §7.4 | MUST-P2 | 否 |
| Node 签名密钥缺失 | P1 | 能力矩阵补充 Node 工作负载签名 key 与 SIGNING_KEY 撤销体系 | §16.3 / §9.4 / §26.1 | MUST-P2 | 是（列入门槛） |
| 同宿主旁路风险 | P1 | Phase 2 强制 Attempt 与 Gate 不同宿主用户 / cgroup / namespace 集合 | §6.6 / §4.1 / §17.3 | MUST-P2 | 否 |
| 工期估算偏乐观 | 计划 | Phase 0～2 修正为 20～28 周，全部章节数字同步 | §0 / §19 / §21 | 文档级 | 是（需决策） |
| 规范范围分层 | 计划 | 新增 MUST-P2 / MUST-P4A 分层表 | §19.2.1 | 规范级 | 是（需决策） |
| 负例样本量统计说服力 | 计划 | Phase 2 / 4A / 4.5 分别要求 N≥40 / 150 / 300 并报告 Clopper–Pearson 单侧上界 | §17.2 / §19.5 / §19.7 / §19.9 | MUST-P2 | 否 |
| 同宿主旁路、73 任务配额来源 | 计划 | 新增 73 任务类别配额表（§17.2.1）；同宿主反例纳入 200 场景 | §17.2× / §19.4 | MUST-P2 | 否 |
| 缺术语表 | 文档 | 新增附录 E 术语表 | 附录 E | 非规范（辅助） | 否 |
| 依赖图不可核验 | 文档 | 新增附录 G Schema 派生 DAG，CI 校验无环 | 附录 G / §17.4 | 非规范（派生视图） | 否 |
| v1.2 继承无力核验 | 文档 | 新增附录 F v1.2 继承清单（ADR-01～24、RLM-001～010 关键句） | 附录 F | 非规范（继承索引） | 否 |
| §5.1 标题与图表述 | 文档 | 标题改为“原子发布”，统一派生图表述 | §5.1 / §3.1 | 文档级 | 否 |
...
**Prime v0.9.1 零网络可行性前提（v1.3.1 明确）：** 整个路线 A 的前提是 Prime/Pi Runtime 能在完全断网、模型 I/O 只走受监督 stdio / 匿名管道（无网络、无 UDS、无可继承 FD、无宿主 Gateway Socket）下稳定运行。该前提必须作为 Phase 0 spike 或 Phase 1 最早的验证项，而不是 Phase 1 末期才发现；若 spike 证明 v0.9.1 在零网络 + stdio-only 下无法运行、无法可靠输出协议事实或依赖隐式网络能力，则路线 A 被阻塞，启用 Native Pi Driver 或保持 ScriptedRuntimeDriver（见 §19.3、§23）。
...
- Reviewer 与 Implementer 满足独立性要求（v1.3.1 定义可编译判定规则，见下）；
- 每个成功路径都能到达确定性 Gate；
- 所有循环都有次数、时间和成本上限；
- 所有外部副作用都由受信任组件执行。

**Reviewer / Semantic Judge 独立性判定规则（v1.3.1 明确，可编译期校验）：** 对每个调用 Semantic Review 或 Reviewer Agent 的 Workflow，编译器必须验证以下全部条件，任一不满足即编译失败（`DIVERSITY_UNSATISFIED`）：

1. Reviewer Run 与 Implementer Run 是不同 Run / Attempt，拥有各自独立的 ExecutionPlanSnapshot 条目；
2. Reviewer 的输入以只读方式绑定选定 Attempt 的不可变输出（ArtifactManifest digest），Reviewer 拥有独立可写工作区且与 Implementer 工作区物理隔离；
3. Reviewer 与 Implementer 使用不同的 rolePackDigest / promptDigest（同一 Prompt / Role Pack 不得同时服务两个对立角色）；
4. 高风险安全评审默认要求 Reviewer 与 Implementer 使用不同 provider / model family，并复用 v1.2 继承清单（附录 F）中的独立性分层规则（不同真实上游厂商、不同模型族、不共享网关故障域；无法满足时显式标记 DIVERSITY_UNSATISFIED，不得静默降级）；
5. 上述各项均以冻结的 AttemptRouteSnapshot / RolePackSnapshot digest 为证据，由 Workflow Compiler 确定性校验，不允许把“人工置信”作为判定输入。
...
**LeasePolicySnapshot（v1.3.1 明确，Phase 0 冻结初值）：** Execution Lease TTL 初值 **30 秒**，Node heartbeat 间隔 **10 秒**，续租须在到期前至少 **10 秒**发起；Verification Lease 与 Merge / GitStaging Lease 的 TTL 在 Phase 0 按作业类型冻结（初值建议 60 秒）。所有 SLO、测试与文档中“2 个 Lease TTL”等单位必须引用同一个已发布 `LeasePolicySnapshot(id, leaseType, ttlSeconds, heartbeatSeconds, renewBeforeSeconds, expiresAtPolicy, payloadDigest, signature)`，禁止各章自行假设数字。
...
**同宿主隔离要求（v1.3.1 明确）：** 即使 Phase 2 单节点部署，“独立 Gate”防假成功的有效性也要求 Attempt 沙箱与 Gate Command 使用**不同的宿主用户、不同的 cgroup 树与不同的 mount / PID / network namespace 集合**；Attempt 进程组不得访问 Gate Supervisor 的通信管道或 Gate 输入目录。“Attempt 篡改 Gate 输入或通信管道”必须作为反例纳入 Phase 2 的 200 个故障 / 安全场景（见 §19.4、§17.3）。
...
**Semantic Review 的信任域归属（v1.3.1 明确）：** Semantic Review Worker 会调用 LLM，因此它不属于 Domain 8。其**编排控制属于 Domain 1**（作为正式 Review Run 创建、调度、预算与审计），**LLM 执行通过独立 Review Run 走 Domain 3 / 4 / 5 链路**（对应用户评审 P1-5 “Semantic Review Worker 会调用 LLM，但 Domain 8 禁止调用 LLM，其所属信任域没标”的修复）；Domain 8 只负责验签、封装 SemanticReviewAttestation 与机械裁决，不调用 LLM、不执行仓库代码。Semantic Review 输入只读接收来自选定 Attempt 的输出与冻结 GatePack，不得成为可写执行环境。
...
发布制品来源（`sourceKind=PUBLISHED_ARTIFACT`，v1.3.1 补充）必须同时满足：对象按内容摘要寻址、携带 SBOM / provenance 与来源链、位于 allowlist 的制品库、工件 digest 与清单一致；不满足任一项则拒绝。该来源与仓库来源一样进入一次性隔离区并形成 sourceSnapshotDigest。
...
Task API 的请求校验发生在 Task 聚合创建前，并把结果写入 `TaskAdmissionDecision`、发布 `TaskAdmissionDecided`；Lifecycle Service 消费决定后创建 Task 状态并发布 `TaskAccepted` / `TaskRejected`。拒绝请求形成 REJECTED Task 与证据。正式迁移如下：

| from | to | 唯一写入者 | guard / proofRefs | event | terminal |
|---|---|---|---|---|---|
| — | ACCEPTED / REJECTED | Lifecycle Service | TaskSpec、准入决定（TaskAdmissionDecision）、idempotency record；Lifecycle 是 TaskAccepted / TaskRejected 唯一发布者 | TaskAccepted / TaskRejected | REJECTED 是 |
...
| VERIFYING | REPAIR_PLANNING | Lifecycle Service | Verdict = REPAIRABLE、Repair 预算允许、失败证据集合与 failureFingerprint 已归档；发布新的 planKind=REPAIR ExecutionPlanSnapshot（§10.5.1） | TaskRepairPlanning | 否 |
| REPAIR_PLANNING | EXECUTING | Lifecycle Service | 签名 planKind=REPAIR ExecutionPlanSnapshot 已发布且逐项继承父计划的不可变输入；获批修理轮次内（Phase 3 默认最多 2 轮） | TaskRepairExecutionStarted | 否 |
| REPAIR_PLANNING | FAILED / NO_VERDICT / HANDOFF_TO_HUMAN | Lifecycle Service | Repair 计划不可发布、Repair 轮次 / 预算耗尽或需人工裁决 | TaskFailed / TaskNoVerdict / TaskHandoffPrepared | 是 |
...
| ACCEPTED / PLANNING / EXECUTING / VERIFYING / REPAIR_PLANNING | CANCEL_REQUESTED | Lifecycle Service | 授权取消命令、当前 rowVersion；停止并 fence 子 Run / Attempt | TaskCancelRequested | 否 |
...
| ACCEPTED / PLANNING / EXECUTING / VERIFYING / REPAIR_PLANNING / DELIVERY_PENDING | QUARANTINED | Lifecycle Service | 安全、来源、证据或撤销证明；外部副作用状态明确 | TaskQuarantined | 是 |
...
| OUTPUT_STAGED | COMMIT_ASSEMBLING | Lifecycle Service | selectedAttempt 候选已 PUBLISHED、CommitIntent 与 operationIdempotencyKey 已分配、CommitBundle 组装完成且签名 CommitBundle 已冻结 | RunCommitAssemblingStarted | 否 |
| COMMIT_ASSEMBLING | VERIFYING | Lifecycle Service | 签名 CommitBundle、EvaluationPlan 已冻结；CommitBundle 是 Verification Plane 产物，必须在进入 Gate 前完成组装 | RunVerificationStarted | 否 |
...
| CREATED / READY / RETRY_WAIT / AWAITING_EXTERNAL / OUTPUT_STAGED / COMMIT_ASSEMBLING / VERIFYING / REPAIR_REQUIRED | CANCELLED | Lifecycle Service | Task 取消已接受；活动 Attempt / Gate 已终止或 fenced；无不明外部副作用 | RunCancelled | 是 |
| CREATED / BLOCKED / READY / EXECUTING / RETRY_WAIT / AWAITING_EXTERNAL / OUTPUT_STAGED / COMMIT_ASSEMBLING / VERIFYING / REPAIR_REQUIRED | QUARANTINED | Lifecycle Service | 安全、来源、证据或撤销证明；活动能力已 fenced，关联外部副作用状态明确 | RunQuarantined | 是 |
...
| TERMINATING | TERMINAL_REPORTED | Node Agent 经 Attempt Service | Node 收到 Stop / 取消 / 超时预警后，提交进程树静默证明与 AttemptTerminalEnvelope；优雅终止允许 Node 上报终态信封后再收敛 | AttemptTerminalReported | 否 |
| TERMINATING | CANCELLED / TIMED_OUT / LOST / FENCED / QUARANTINED | Lifecycle Service | 强制终止路径：Node 失联（Lease 过期、Node fencing / epoch 提升证明）或 Stop 后仍未在到期时限内提交信封 | 对应强制终态事件 | 是 |
...
subjectType = EVALUATION_POLICY_TEMPLATE | ROUTING_POLICY_TEMPLATE | MODEL_POLICY | SANDBOX_PROFILE | RUNTIME_IMAGE | SANDBOX_IMAGE | DEPENDENCY_IMAGE | EXECUTION_IMAGE_COMPATIBILITY | SKILL_BUNDLE | GATE_PACK | PROMPT | TOOL_POLICY | ROLE_PACK | WORKFLOW_POLICY_TEMPLATE
...
**模板与实例边界（v1.3.1 明确）：** 本节的 subjectType 一律审批“政策模板/政策对象”，不审批逐任务实例。`EVALUATION_POLICY_TEMPLATE` 的 subject 是 EvaluationPlanTemplate，`ROUTING_POLICY_TEMPLATE` 的 subject 是 RoutingIntentTemplate，`GATE_PACK` / `PROMPT` / `TOOL_POLICY` / `ROLE_PACK` / `WORKFLOW_POLICY_TEMPLATE` 的 subject 分别是对应不可变快照。由模板与 TaskSpec 确定性编译出的任务实例（EvaluationPlanSnapshot、RoutingIntentSnapshot、ExecutionPlanSnapshot）**不逐任务走 Four-Eyes**，但必须携带 `templateRef / templateDigest`、`compilerId / compilerVersion` 与派生输入 digest，并满足“实例 digest 可重算、引用模板已批准且未撤销、实例不超出模板允许范围”三条件；任一不满足则编译产物视为无效，不得进入 Attempt。GatePack 定义“什么算成功”，属于全系统最安全敏感的政策对象，其审批、quorum 与 veto 必须与其余 subjectType 同样走本节合同，并禁止降低 required Gate 基线。
...
`approvalRefs[]` 必须引用 `subjectType=ROLE_PACK` 的 PolicyApprovalSet 及其完整签名 Decision 集，且至少包含功能（OWNER_APPROVER）与安全（SECURITY_APPROVER）两类；Role Pack 组合的 buildInputsDigest 必须等于 PolicyApprovalScope 绑定内容，任何 Role / Prompt / Tool / Skill 组合变化都创建新 RolePackSnapshot 与新审批，不原地修改。
...
**AttemptContract 唯一 Schema 真身（v1.3.1 明确）：** §10.6 的扁平字段模型是 AttemptContract 的唯一 Schema 真身。附录 B 的正例必须逐字段与本节一致，采用与本节列表相同的扁平嵌套层级，不得出现 `frozenRefs`、`runtime`、`source`、`security`、`budget` 等旁路分组，不得重复声明 `gitPolicy` / `security.gitRemote*` 或 `runtimePolicy` 中的第二份 `rlmForWriteAttempt`、`modelCallMode`。附录 B 必须由 canonical Schema fixture 生成或在 CI 中对同一 Schema 校验（见 §17.4）；任何人手维护的第二个结构都被视为未被 Schema 接受的无效示例。附录 B 不得包含 `revocationEpochSet`、`skillEpoch`、`routeEpoch`、`credentialEpoch`；撤销水位只有单一全局 `revocationEpoch`（见 §10.7）。
...
只有 Node Runtime Proxy 可以调用 InvokeModel。请求携带稳定 `invocationId`、`providerRequestId`（获知后补记）、AttemptContract、AttemptRouteSnapshot、requestSequence、requestDigest，以及 Node 持有的 LeaseGrant / GatewayGrant。Gateway 校验签名与 TTL、双 epoch、已完整同步的撤销水位、**BudgetGrant（本地单调扣减，不因每次调用同步查询 PostgreSQL；见 §18.3）** 和实际 provider / model / thinking；同 invocationId 不重复结算或发起第二次逻辑调用。
...
~~~text
TaskAdmissionDecided.v1      # Task API 发布（唯一发布者）
TaskAccepted.v1              # Lifecycle 消费 TaskAdmissionDecided 后发布（唯一发布者）
TaskRejected.v1              # Lifecycle 发布（唯一发布者）
RunReady.v1
AttemptClaimed.v1
...
3. Workflow Compiler 从已批准 Workflow 政策模板（Four-Eyes）与 TaskSpec 确定性编译不可变 DAG 与 EvaluationPlan 任务实例（携 templateRef / templateDigest、compilerId / compilerVersion 与派生输入 digest；见 §7.7 / §10.3.1，不逐任务人工审批）；
4. Route Resolver 从已批准 Routing 政策模板冻结 RoutingIntent / AttemptRoute；Registry 解析 Role / Skill / Prompt / Tool / Model / Gate / Sandbox 快照；
...
#### 17.2.1 73 个正向任务的配额来源（v1.3.1 明确）

“73”的推导来源为下列配额表，类别 ×（语言/框架）× 难度分档合计严格等于 73；Phase 0 在冻结时按表填满并逐项记录输入基线、Oracle 与风险等级：

| 类别 | 语言 / 框架分档 | 难度 | 数量 |
|---|---:|---:|---:|
| 确定性缺陷修复 | Go / Python / TypeScript × 2 框架 | 低 | 18 |
| 确定性缺陷修复 | Go / Python / TypeScript × 2 框架 | 中 | 12 |
| 测试补充（缺失分支 / 边界覆盖） | Go / Python / TypeScript | 低～中 | 15 |
| 小型功能（边界明确、可自动化验收） | Go / Python / TypeScript | 中 | 16 |
| 重构（行为保持，Oracle 重放） | Go / Python / TypeScript | 中 | 6 |
| 权限 / 路径 / 数据分级合规修复 | 任意主流语言 | 高 | 6 |
| **合计** | | | **73** |

负例集配额：模糊规格 ≥ 12、安全诱导（Prompt Injection / 后门 / 越权）≥ 12、超预算 / 超时 ≥ 6、Git 漂移 / 竞态 ≥ 6、Skill / Route / Evidence 漂移 ≥ 4，合计 ≥ 40。任何类别无法完成时必须在冻结评审会上说明，不允许用另一类凑数抬高名义配额。
...
**统计口径（v1.3.1 明确）：** UnsafeAcceptanceRate 的区间统一采用 **Clopper–Pearson（或等价预注册方法）** 计算，0/N 时单侧 95% 上界 ≈ `1 - 0.05^(1/N)`。各阶段负例门槛：Phase 2 要求 `N ≥ 40`（0/40 对应上界约 **7.2%**）；Phase 4A 要求 `N ≥ 150`（上界约 **2.0%**）；Phase 4.5 要求 `N ≥ 300`（上界约 **1.0%**）。观测为零绝不等于真实风险为零，报告必须同时给出 N 与上界；阶段门槛以“时间与样本量较晚满足者”为准。
...
~~~text
估算 → Ledger 预留签发 BudgetGrant → Gateway 本地单调扣减 → 异步结算回 Ledger
              ↓
        80% Warning
        90% 降低非必要工作
       100% 硬停止
...
**BudgetGrant 合同（v1.3.1 明确，解决“模型调用热路径不允许同步查询 PostgreSQL”与“每次调用前检查预算”的冲突）：**

1. Ledger Service 为 Attempt 预留额度并签发不可变的签名 `BudgetGrant(budgetGrantId, attemptId, executionPlanDigest, ledgerEpoch, reservedTotalMicros, perCallCapMicros, maxInvocations, grantSeqStart, grantSeqEnd, validFrom, expiresAt, budgetPolicyDigest, payloadDigest, signature)`；
2. Gateway 收到 Node Runtime Proxy 代转的模型调用时，**只读取持有的 BudgetGrant 并在本地单调扣减**（调用序号、累计金额、Token 上限），不因每次调用同步查询 PostgreSQL；
3. 每次调用前 Gateway 校验：Grant 签名与 TTL 有效、调用序号在 [grantSeqStart, grantSeqEnd] 内、累计余额 ≥ perCallCapMicros、合同与撤销水位一致；任一不满足即拒绝并 Fail Closed；
4. 用量（含实际 Token 与费用）异步结算到 Ledger Service，按 invocationId 幂等；Ledger 是跨 Worker 崩溃的唯一预算权威；
5. Grant 过期、同步落后、余额不足、重放或对账不明时 Gateway 拒绝新调用；预算硬停止不依赖模型热路径上的数据库查询；
6. Node 侧不再逐调用鉴权预算，只核对 Grant 的合法性视图；Attempt 与 Runtime 始终不持有 Grant。
...
| 阶段 | 周期 | 核心结果 | 最高可声明成熟度 |
|---|---:|---|---|
| Phase 0 | 4～6 周 | 架构、合同、基准、责任冻结；Prime 零网络 spike（§19.3） | 可实施设计 |
| Phase 1 | 5～7 周 | Runtime、安全边界、状态机和 Skill 供应链 PoC | 受控 PoC |
| Phase 2 | 7～10 周 | PUBLIC 单节点候选分支闭环 | 内部 MVP |
| Phase 3 | 7～9 周 | 多角色和有界 Repair 产生量化收益 | 增强型 MVP |
| Phase 4A | 9～13 周 | INTERNAL、MicroVM、专用节点与备份恢复 | 可恢复生产候选 |
| Phase 4B（可选） | 5～8 周 | 独立故障域、同步复制与真实 HA | 高可用生产候选 |
| Phase 4.5 | 8～11 周 | Shadow 与分档 Canary；时间和样本量取较晚满足者 | 有限生产可用 |
| Phase 5 | 持续 | 独立演进、评测、撤销和优化 | 持续运营 |

> 阶段周期已按 v1.3 第三方评审修正：Phase 0～2 的名称时长合计 **16～23 周**；考虑跨阶段集成、评审、缓冲与并行调度，规划工期取 **20～28 周**（§0、§21 同口径），不再使用原 13～19 周的旧估计。阶段门槛不能跳过。

### 19.2.1 MUST-P2 / MUST-P4A 规范分层（v1.3.1 新增）

同一逻辑约束按“兑现阶段”标注规范等级，避免把 Phase 4A 级别的供应链分权细节全部压到 Phase 2 之前：

| 约束族 | 规范等级（Phase 2） | 说明 |
|---|---|---|
| 唯一字段级写入者、状态机 CAS、摘要寻址、Fail Closed、完全断网、撤销 Overlay 与单调 epoch、无自动合并、证据完整性 | **MUST-P2** | 不变量，任何阶段不得省略 |
| Builder / Approver / Signer / Publisher 身份分离的“逻辑不变量” | MUST-P2 | Phase 2 可用同一 Signer 服务 + 多把 key + 独立审计记录实现，但不得由同一身份兼任相邻风险职责 |
| 独立“服务进程级”Builder/Approver/Signer/Publisher 分权部署 | MUST-P4A | Phase 4A 拆分为独立服务身份与独立密钥存储 |
| DependencyImage / ExecutionImage 供应链 Candidate → Snapshot → Publication 三件套 | MUST-P4A | Phase 2 可先以“未签名 Snapshot + 签名 + 发布指针”最小集合过渡，但发布前必须有审批与签名 |
| INTERNAL 数据 MicroVM + 专用节点 | MUST-P4A | Phase 2 仅 PUBLIC / 已脱敏 |
| 多 AZ / 跨域复制 / 独立故障域 | MUST-P4B（可选） | 无物理条件时保持“可恢复、非 HA”声明 |
| Shadow / Canary 分档发布 | MUST-P4.5 | Phase 2 只离线评测与盲态 A/B 基础 |

任何复审、验收或文档引用“该约束必须满足”时，必须同时指明其 MUST 等级；若某条目未标注，默认按 MUST-P2 执行。
...
- **Prime v0.9.1 零网络 spike 结论（v1.3.1 新增，见 §3.4）：** 在完全断网、模型 I/O 仅经受监督 stdio / 匿名管道条件下，验证 v0.9.1 能启动、能经 Driver 管道报告 ModelCallIntent、能输出可靠终态 / 静默事实。结论为“可行”才允许继续路线 A；结论为“不可行或依赖隐式网络能力”则触发 Native Pi Driver / ScriptedRuntimeDriver 切换评估，该项未关闭前不得签署 Phase 0 Go；
- 至少 20% 为未参与调优的保留集；
...
5～7 周；Phase 0 全部硬门槛通过（含 Prime 零网络 spike），Linux x86_64 安全实验节点可用。
...
7～10 周；Phase 1 通过，PostgreSQL、MinIO/S3、一个 Git Provider、Model Gateway 和单节点环境就绪。
...
- ExpectedOutcomeMatch ≥ 95%；UnsafeAcceptanceRate = 0/N（N ≥ 40，采用 Clopper–Pearson，0/40 单侧 95% 上界 ≈ 7.2%，报告必须给出 N 与上界）；
- Evidence Completeness：100%；
- False Success：0；
...
7～9 周；Phase 2 稳定通过，并积累真实失败分类和人工复核样本。
...
9～13 周；Phase 3 通过，身份、KMS / Secret Broker、数据治理和安全复核资源到位。不要求独立故障域，因此本阶段不得宣称 HA。
...
- INTERNAL 正向基准至少 100 个任务 × 3 次，PositiveTaskE2ESR ≥ 70%，任一类别 ≥ 50%，对应负例 UnsafeAcceptanceRate = 0/N（N ≥ 150，Clopper–Pearson 单侧 95% 上界 ≈ 2.0%）。
...
5～8 周；Phase 4A 通过，并且至少两个真实独立故障域、同步数据库复制和跨域对象存储可用。
...
8～11 周；Phase 4A 全部硬门槛通过，真实任务流、对照组、值守和一键回退可用。只有业务明确要求 HA 时才依赖 Phase 4B。Shadow 可在 Phase 4A 后四周提前累计，但只能在安全边界已冻结且外部写入为 0 时计算；否则顺序执行。每个门槛均以“最短时间与最小样本量较晚满足者”为准。
...
上述 20～28 周（Phase 0～2，v1.3.1 评审修正口径）估算以 **至少 5 名核心 FTE + 独立评测 / 安全投入** 为前提，并确保以下职责实际有人承担：
...
| Prime 在完全断网 + stdio-only 下不可运行 | 高 / 极高 | Phase 0 spike 失败或协议事实不稳定 | 提前 spike；切换 Native Pi Driver / ScriptedRuntimeDriver；不推迟验证 | Runtime |
| 评审材料（PR/MR）外部副作用不明确 | 中 / 中 | 候选 ref 与 PR 对象漂移 | Phase 2 默认不建 PR；启用时定义 CreateReviewRequest 幂等合同与读回 | Integration |
| 同宿主 Gate 输入被 Attempt 篡改 | 中 / 极高 | 旁路测试 / 审计发现交叉访问 | 不同宿主用户、cgroup、namespace；专项反例入 200 场景 | Security |
| 规范范围超过团队交付能力 | 高 / 高 | 阶段滑期、验收积压 | MUST-P2 / MUST-P4A 分层；20～28 周工期基线；每阶段削减评审 | Owner |
...
- **Prime v0.9.1 零网络 spike 结论已产生并记录（§19.3）；Node 对 AttemptTerminalEnvelope 的签名密钥已纳入能力矩阵与 SIGNING_KEY 撤销体系（§16.3）**；
- Phase 1 预算、节点和责任人已经落实；
- 所有未决项有 Owner、截止日期和阻塞级别（详见第 28 章评审整改闭环表与未决项清单）。
...
本表用于复审逐项签字：每一条评审意见必须有处置状态、责任章节、验证方法与闭环签字。P0 项在 v1.3.1 中已全部修订为正文条款；P1 项中已在正文关闭的标注“已修订”，其余作为未决项进入 §27.1 门槛清单，必须带 Owner 与截止日期。

| 评审编号 | 意见摘要 | 级别 | 处置状态 | 修订章节 / 未决项载体 | 验证方法 | 复审签字 |
|---|---|---|---|---|---|---|
| P0-1 | 撤销水位模型全局 vs 分域不一致 | P0 | 已修订（全局单一 revocationEpoch） | §10.6 / §10.7 / 附录 B | CI 断言无分域水位字段；附录 B 经 Schema 正例校验 |  |
| P0-2 | 附录 B 与 §10.6 结构 / 枚举冲突 | P0 | 已修订（附录 B 扁平化，与 Schema 一致） | 附录 B / §10.6 / §17.4 | JSON parse + Schema 正例 + digest 测试向量 |  |
| P0-3 | Task 状态机缺 Repair 回路 | P0 | 已修订（VERIFYING → REPAIR_PLANNING → EXECUTING） | §8.1 / §8.2 / §14.3 / §10.5.1 | 状态机模型测试覆盖该回路与终态收敛 |  |
| P0-4 | TaskAccepted / TaskRejected 双发布者 | P0 | 已修订（Task API 只发 TaskAdmissionDecided） | §6.1 / §8.1 / §13.6 | 事件目录唯一发布者断言 |  |
| P0-5 | 政策 Four-Eyes 与任务实例混用 | P0 | 已修订（模板审批 vs 实例确定性编译） | §7.7 / §10.3.1 / §14.2 | 编译器产出实例携 templateRef / compiler digest；模板变更审计 |  |
| P0-6 | 预算检查与热路径约束冲突 | P0 | 已修订（BudgetGrant 本地扣减 + 异步结算） | §12.5 / §13.3 / §18.3 / §26.1 | 性能测试证明热路径无同步 DB 查询；Grant 重放 / 过期反例 |  |
| P0-7 | GatePack / Prompt / ToolPolicy / RolePack / Workflow 缺审批对象 | P0 | 已修订（subjectType 扩容 + approvalRefs 合同） | §10.3.1 / §10.5 / §26.1 | Schema 枚举测试向量 |  |
| P1-1 | Prime 零网络可行性前置 | P1 | 已修订为 MUST-P2 门槛 | §3.4 / §19.3 / §23 | Phase 0 spike 报告 |  |
| P1-2 | 候选分支评审材料（PR/MR）语义 | P1 | 已修订（Phase 2 默认不建 PR） | §3.5 / §12.8 / §19.5 / §23 | 幂等合同测试 |  |
| P1-3 | Reviewer 独立性无可编译判定 | P1 | 已修订（5 条编译期规则） | §6.3 / §19.6 | Workflow Compiler 单测 |  |
| P1-4 | candidate ref 多轮 Repair CAS 语义 | P1 | 已修订 | §8.5 / §10.10 | 状态机 + Git 集成测试 |  |
| P1-5 | Semantic Review 信任域未标 | P1 | 已修订（编排属 Domain 1，LLM 走独立 Review Run） | §4 / §6.10 | 部署拓扑断言 |  |
| P1-6 | Commit Assembler 时序缺口 | P1 | 已修订（新增 COMMIT_ASSEMBLING） | §8.2 / §6.10 | 状态机测试 |  |
| P1-7 | Attempt TERMINATING 优雅终止路径 | P1 | 已修订（TERMINATING → TERMINAL_REPORTED） | §8.3 / §12.2 | 状态机测试 |  |
| P1-8 | 任务白名单离线能力要求 | P1 | 已修订 | §1.2 / §6.14 / §19.5 | 准入检查单测 |  |
| P1-9 | Lease TTL 未冻结 | P1 | 已修订（LeasePolicySnapshot 初值） | §6.5 / §18.2 / §19.3 | SLO 引用同一策略对象 |  |
| P1-10 | SkillPackageVersion.sourceKind 枚举 | P1 | 已修订（增加 PUBLISHED_ARTIFACT） | §10.1 / §7.4 | Schema 枚举测试 |  |
| P1-11 | Node 签名密钥缺失 | P1 | 已修订（能力矩阵 + SIGNING_KEY 撤销体系） | §9.4 / §16.3 / §26.1 | 签名 / 撤销反向索引测试 |  |
| P-工期 | 工期估算偏乐观 | 计划 | 已修订（Phase 0～2 = 20～28 周） | §0 / §19 / §21 | 阶段计划一致性校验 |  |
| P-分层 | 规范范围无分层 | 计划 | 已修订（MUST-P2 / MUST-P4A 分层表） | §19.2.1 | 各约束带等级标注 |  |
| P-样本 | 负例样本量统计 | 计划 | 已修订（40 / 150 / 300 + Clopper–Pearson） | §17.2 / §17.3 / §19.5 / §19.7 / §19.9 | 统计报告模板 |  |
| P-配额 | 73 任务无配额来源 | 计划 | 已修订（17.2.1 配额表合计 73） | §17.2.1 | 配额表复算 |  |
| P-旁路 | 同宿主旁路风险 | 计划 | 已修订（隔离要求 + 反例入 200 场景） | §6.6 / §4.1 / §19.4 | 故障注入场景覆盖 |  |
| W-术语 | 缺术语表 | 文档 | 已修订（附录 E） | 附录 E | 术语一致性抽查 |  |
| W-依赖图 | 依赖图不可核验 | 文档 | 已修订（附录 G + CI 无环校验） | 附录 G / §17.4 | CI 反环测试向量 |  |
| W-继承 | v1.2 无力核验 | 文档 | 已修订（附录 F 继承清单） | 附录 F | 引用完整性抽查 |  |
| W-表述 | §5.1 标题与图表表述 | 文档 | 已修订 | §5.1 / §3.1 | 文档走读 |  |

未决项（进入 §27.1，必须带 Owner / 截止日期）：

| 未决项 | Owner（建议） | 截止 | 阻塞级别 |
|---|---|---|---|
| Prime v0.9.1 零网络 spike 结论 | Runtime | Phase 0 第 15 天 | 阻塞路线 A 决策 |
| Node 签名密钥的 KMS 存储与轮换方案 | Security | Phase 0 冻结日 | 阻塞节点终态签名 |
| Phase 4.5 负例集扩充至 ≥ 300 的预算与红队排期 | Quality | Phase 4A 开始日 | 阻塞 4.5 门槛 |
| PR/MR 自动创建启用 ADR（如目标周期内需要） | Integration | Phase 2 评审会 | 不阻塞默认路径 |
...
以下正例由 §10.6 AttemptContract 唯一 Schema 真身生成（§10.6 / §17.4）；字段层级、枚举与正文逐项一致，撤销水位只有单一全局 `revocationEpoch`，不含任何分域水位。按 §9.2 的版本规则（major.minor），Schema 版本为 `1.3`；文档发布版本 v1.3.1 在 schema meta 之外单独表达，不写入 Schema 版本号。示例中所有 `<...>` 占位符（如 `<execution-plan-digest>`、`<git-object-hex>`）仅为文档展示，进入 CI/DigestProfile 正例测试前，必须由测试夹具替换为 §9.3 规定的 `sha256:<64 位小写十六进制>` 合法值，且替换后逐字节过 Schema——本示例本身不声称通过真实 digest 校验。
...
评审整改闭环表见本文第 28 章；未决项清单及其 Owner / 截止日期见 §27.1 与第 28 章。
...
| 术语 | 定义 |
|---|---|
| Snapshot | 不可变、可寻址、签名后的政策 / 运行对象（如 RolePackSnapshot、ExecutionPlanSnapshot）；变化创建新版本 |
| Template | 可复用、须 Four-Eyes 审批的“政策模板”（如 EvaluationPolicyTemplate）；任务实例由模板确定性编译 |
| Candidate | 尚未获得最终审批或发布状态的对象（如 ExecutionImageCompatibilityCandidate），不可用于 Attempt / Gate |
| Publication | 指向已批准 Snapshot 的可变发布状态与环境作用域指针；Snapshot 本身不含发布状态 |
| Attestation | 由独立服务对精确事实摘要的签名证明（如 RouteAttestation、SemanticReviewAttestation），不是裁决 |
| Authorization | 向受信任服务签发的最小权限执行授权（如 DeliveryAuthorization），绑定主体、动作、对象、epoch 与 TTL |
| ApprovalSet | 对独立签名 Decision 集的机械聚合结果，携带 quorum / veto / 身份独立性结论（含 SkillApprovalSet 与 PolicyApprovalSet） |
| Grant | 短时、绑定上限并可单调扣减的额度 / 能力凭证（LeaseGrant、GatewayGrant、BudgetGrant） |
| Lease | 执行 / 验证 / 合入作业的所有权凭证，绑定 owner、epoch、TTL（ExecutionLease、VerificationLease、GitStagingLease 等） |
| Epoch | 单调递增的分布式一致性水位（controlPlaneEpoch、resourceExecutionEpoch 与全局 revocationEpoch） |
| revocationEpoch | 全局唯一撤销水位；任何合同 / Grant / 授权要求的水位与本地水位精确相等才可处理 |
| Fencing Token / Fencing | 阻止过期所有者写入或产生副作用的机制 |
| CommitBundle | 由 Commit Assembler 组装的不可变 Git 提交对象包，预计算 tree / commit GitObjectId |
| CAS | Compare-And-Swap；以 expectedState / expectedRowVersion / expectedRefGitObjectId 做条件更新 |
| Fail Closed | 证据或水位不足时拒绝而非放行（默认拒绝） |
| NO_VERDICT | 无法形成可靠裁决的结果，不得解释为成功 |
| MUST-P2 / MUST-P4A | 规范分层等级：P2 为 Phase 2 前必须满足的不变量；P4A 为 Phase 4A 粒度要求 |

## 附录 F：v1.2 继承清单（关键条款摘录）

v1.2 与 v1.1 保留不改；本清单把 v1.3.1 依赖的 v1.2 关键条款以摘录形式固化，避免仅凭外部章节号引用而无法核验。以下条款视为 v1.3.1 规范输入，与正文冲突时以正文 c1.3.1 明确面为准：

- **ADR-01（OUTPUT_STAGED 写入权）**：Attempt.state=OUTPUT_STAGED 由 Lifecycle Service 写入，守卫为 TERMINAL_REPORTED + ArtifactManifest 已 CAS 发布 + rowVersion。
- **ADR-02（系统强制终态写入者）**：LOST / FENCED / TIMED_OUT 由 Lease / Lifecycle 以数据库时间与 fencing 证明写入，不依赖失联 Node。
- **ADR-06（Gate 结果捕获点）**：Gate 退出码由沙箱外 waitpid 捕获，容器内结果文件只作解释证据，不参与裁决。
- **ADR-07（RLM 信号降级）**：控制面不依赖 Prime 内部类型；能力协商失败、RLM 未静默时输出 NO_VERDICT。
- **ADR-08（RLM 与危险解释器）**：写 Attempt 禁用 RLM；只读 Attempt 启用深度 1 必须无危险解释器能力。
- **ADR-14（Merge 漂移判定）**：Merge Saga 处理“自身 ack 丢失”与“真实外部漂移”之前必须前置读回目标 ref。
- **ADR-15（安装脚本与 MicroVM）**：需执行安装脚本的任务使用 MicroVM；v1.3.1 Phase 2 白名单要求全部 Gate 离线、依赖预构建（§1.2）。
- **ADR-16（10 信任域）**：v1.2 扩展的 10 个信任域（含 Node Agent 跨边界解析层、Model Gateway 拆分）为 v1.3 的 13 域基础。
- **ADR-17（取消路径完备）**：任意非终态可接收取消 / 隔离；MERGING@APPLIED 之后不可逆。
- **ADR-19（渐进式成功）**：PARTIALLY_VERIFIED / DEGRADED_SUCCESS / CONDITIONAL_SUCCESS 的条件须预先冻结。
- **ADR-21（预算异步）**：预算为跨 Worker 崩溃的权威；v1.3.1 以 BudgetGrant 落实热路径不查询 PG（§18.3）。
- **ADR-23（Trace 契约）**：traceId 贯穿 Task 全生命周期，Attempt 沙箱内代码不能修改。
- **ADR-24（重试 / 熔断参数表）**：阈值参数须参数化并写进测试。
- **RLM-001～RLM-010（静默屏障与回归）**：读任务 RLM 静默屏障与恢复完整性验证在 v1.3 中以“强终态屏障 + 证据完整性”形式继承；写 Attempt 一律禁用。
- **v1.2 §10.3 硬独立性**：不同 provider 名称不等于独立路线；高风险评审要求不同真实上游、不同模型族、不共享网关故障域。v1.3.1 §6.3 将其编译期为 5 条可校验规则。

（其余 v1.2 / v1.1 条款仍按“保留不改”生效；本清单仅摘录 v1.3.1 依赖最深的条款，供复审核对。）

## 附录 G：Schema 派生对象依赖 DAG（非规范性派生视图）

由各核心 Schema 的引用字段派生，CI 按 §17.4 校验无环并核对与 Schema 一致。本图是派生视图，不得新增或改变权威关系。

```text
TaskAdmissionDecision ──► Task ──► ExecutionPlanSnapshot ──► AttemptRouteSnapshot ──► AttemptContract
                                      │                          ▲
                                      ├──► WorkflowSnapshot ──────┤
                                      ├──► EvaluationPlanSnapshot ┘
                                      ├──► RolePackSnapshot ──► SkillBundleSnapshot ──► SkillPackageVersion
                                      │        └──► approvalRefs → PolicyApprovalSet ──► PolicyApprovalDecision
                                      ├──► GatePackSnapshot / Prompt / ToolPolicy / ModelPolicy
                                      ├──► SourceBundle ──► DependencyImageSnapshot
                                      ├──► Runtime/Sandbox Image Snapshot ──► ExecutionImageCompatibilitySnapshot
                                      └──► BudgetGrant ──► Budget Ledger

AttemptContract ──► Attempt ──► AttemptTerminalEnvelope（Node 签名）
Attempt ──► ArtifactManifest ──► CommitIntent ──► CommitBundle ──► GateExecution ──► GateAttestation
          ──► SemanticReviewAttestation ──► EvaluationVerdict ──► PreDeliveryEvidence
          ──► DeliveryAuthorization ──► CandidateStagingOperation ──► GitStagingResult
          ──► DeliveryEvidenceBundle ──► Task SUCCEEDED

RevocationRecord ──(subjectDigest)──► 全部可撤销对象；全局 revocationEpoch 单调推进
```

CI 要求：本图引用的每个箭头都能在对应 Schema 的引用字段中找到；反环测试向量（§10.3.1）通过；任何 Schema 变更导致图不一致时构建失败。
